...
def build_trainer(
    ...
    class trainer_cls(base):
        ...
        def _init(self, config, env_creator):
            ...
            w1 = RolloutWorker(
                env_creator=lambda _: gym.make("Webots-v0"),
                policy=PPOTFPolicy)
            w2 = [
                RolloutWorker.as_remote().remote(
                    env_creator=lambda c: gym.make("Webots-v1"), policy=PPOTFPolicy),
                RolloutWorker.as_remote().remote(
                    env_creator=lambda c: gym.make("Webots-v2"), policy=PPOTFPolicy)
            ]
            # w2 = [
            #     RolloutWorker.as_remote().remote(
            #         env_creator=lambda c: gym.make("Webots-v1"), policy=PPOTFPolicy)
            ...
            # ]
            workers = WorkerSet._from_existing(w1, w2)
